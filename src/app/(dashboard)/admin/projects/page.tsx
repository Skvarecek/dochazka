...
import { Plus, X, Building2, MapPin, Trash2, Eye, EyeOff } from "lucide-react";
...
export default function ProjectsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setFormName(""); setFormAddress(""); setFormDesc(""); setShowForm(false);
    loadData();
    setSubmitting(false);
  }
  ...
  async function deleteProject(id: string) {
    if (!confirm("Opravdu smazat zakázku? Záznamy hodin zůstanou, jen nebudou přiřazeny k zakázce.")) return;
        ...
        <div className="card p-6 animate-in">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="label">Název zakázky</label>
                <input type="text" className="input" value={formName} onChange={e => setFormName(e.target.value)} placeholder="Střecha Novákovi" required />
              </div>
              <div>
                <label className="label">Adresa</label>
                <input type="text" className="input" value={formAddress} onChange={e => setFormAddress(e.target.value)} placeholder="Písek, Budějovická 12" />
              </div>
            </div>
            <div>
              <label className="label">Popis</label>
              <textarea className="input min-h-[60px] resize-y" value={formDesc} onChange={e => setFormDesc(e.target.value)} placeholder="Kompletní rekonstrukce střechy..." />
            </div>
            <button type="submit" disabled={submitting} className="btn-primary">{submitting ? "Ukládání..." : "Vytvořit zakázku"}</button>
          </form>
        </div>
      )}
      ...
      <div className="space-y-3">
        {projects.length === 0 ? (
          <div className="card px-6 py-12 text-center text-ink-500">
            <Building2 className="w-12 h-12 mx-auto mb-3 text-ink-300" />
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {!project.is_active && <span className="badge bg-surface-200 text-ink-500">Neaktivní</span>}
                  </div>
                  {project.address && (
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => toggleActive(project.id, project.is_active)} className="btn-secondary p-2" title={project.is_active ? "Deaktivovat" : "Aktivovat"}>
                    {project.is_active ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                  <button onClick={() => deleteProject(project.id)} className="btn-secondary p-2 hover:text-red-500" title="Smazat">
